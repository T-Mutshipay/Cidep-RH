<x-app-layout>
    ...
    <div class="m-12">
        ...
        <h1 class="text-xl font-bold">Gestion des Directions</h1>
        ...
        <div id="createForm" class="hidden">
            <h2 class="text-lg font-semibold">Créer une Direction</h2>
            <form action="{{ route('directions.store') }}" method="POST">
                @csrf
                <div class="mb-4">
                    <label for="nom_direction" class="block">Nom de la Direction:</label>
                    <input type="text" name="nom_direction" id="nom_direction" required class="border rounded p-2 w-full">
                </div>
                <div class="mb-4">
                    <label for="code_direction" class="block">Code de la Direction:</label>
                    <input type="text" name="code_direction" id="code_direction" required class="border rounded p-2 w-full">
                </div>
                ...
                <tbody>
                    @foreach($directions as $direction)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                            ...
                            <td class="px-6 py-4">{{ $direction->code_direction }}</td>
                            <td class="px-6 py-4">
                                <a href="{{ route('directions.edit', $direction->id) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                                    <i class="fas fa-edit"></i> Éditer
                                </a>
                                
                                <button type="button" 
                                    data-url="{{ route('directions.destroy', $direction->id) }}" 
                                    onclick="supprimer(event);"
                                    data-modal-target="popup-modal" 
                                    data-modal-toggle="popup-modal"
                                    data-message="Êtes-vous sûr de vouloir supprimer La direction de {{ $direction->nom_direction }} ?"
    ...
    <script>
        ...
        //         var url = event.target.getAttribute('data-id');
        //         alert(url);
        //     });
        // });
